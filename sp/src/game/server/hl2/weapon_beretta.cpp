...
public:
	...
	{		
		...
		if ( beretta_use_new_accuracy.GetBool() )
		{
...
acttable_t *GetBerettaActtable()
{
	return CWeaponBeretta::m_acttable;
}

int GetBerettaActtableCount()
{
	return ARRAYSIZE(CWeaponBeretta::m_acttable);
}
#endif
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponBeretta::FireNPCPrimaryAttack( CBaseCombatCharacter *pOperator, Vector &vecShootOrigin, Vector &vecShootDir )
{
	CSoundEnt::InsertSound( SOUND_COMBAT|SOUND_CONTEXT_GUNFIRE, pOperator->GetAbsOrigin(), SOUNDENT_VOLUME_PISTOL, 0.2, pOperator, SOUNDENT_CHANNEL_WEAPON, pOperator->GetEnemy() );
...
//-----------------------------------------------------------------------------
void CWeaponBeretta::Operator_ForceNPCFire( CBaseCombatCharacter *pOperator, bool bSecondary )
{
...
#ifdef MAPBASE
ConVar weapon_beretta_upwards_viewkick( "weapon_beretta_upwards_viewkick", "0" );
#endif
...
#ifdef MAPBASE
	viewPunch.x = weapon_beretta_upwards_viewkick.GetBool() ? random->RandomFloat(-0.5f, -0.25f) : random->RandomFloat(0.25f, 0.5f);
#else
